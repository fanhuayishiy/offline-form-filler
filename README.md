...
- 🧩 **参数完全自由**:新建套只带姓名/手机/邮箱基础模板;所有参数可点击改名、可删除(改名自动同步识别词与映射绑定);
...
1. 点扩展图标 →「我的资料」录入姓名、手机、地址等 → 保存(新建资料套只带姓名/手机/邮箱基础模板;**参数名可点击改名、参数可删**,改名自动同步识别词与映射绑定);
2. 打开表单页面:自动填充开关开着即自动填;或点「⚡ 填充当前页面」/ 输入框右键「⚡ 一键填充表单」;
3. 识别不了的字段:右键 →「🎯 设置此字段」→ 下拉绑定资料参数或填固定值;
4. 填错了:弹窗「填充历史」→「↩ 撤销最近一次填充」;
5. 换环境:「⬇ 导出」→ 新浏览器里「⬆ 导入」;批量建套用「📋 CSV」或「📊 Excel」。
